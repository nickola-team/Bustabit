<tr>
	...
	<td>
		...
		<button type="button" class="btn btn-block btn-success btn-sm">충전</button>
		</a>
		@else
			<button type="button" class="btn btn-block btn-success disabled btn-sm">충전</button>
		@endif
	</td>
	<td>
		@if( Auth::user()->hasRole('admin'))
		<a class="outPayment" href="#" data-toggle="modal" data-target="#openOutModal" data-id="{{ $jackpot->id }}" >
		<button type="button" class="btn btn-block btn-danger btn-sm">환전</button>
		</a>
		@else
			<button type="button" class="btn btn-block btn-danger disabled btn-sm">환전</button>
		@endif
	</td>
</tr>